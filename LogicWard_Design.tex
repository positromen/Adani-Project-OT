...
9 & Two-host integration on real Pi + laptop & verified on a Pi 4 + laptop over Wi-Fi \\
\bottomrule
\end{tabularx}

\smallskip
\textbf{Delivered: 98/98 checks across 7 suites, plus a verified real-hardware two-machine
run} --- a Raspberry Pi 4 running the Modbus PLC + sensor agent, a laptop running the engine
+ SOC dashboard, with live cyber detection (laptop$\rightarrow$Pi) and agent event-forwarding
(Pi$\rightarrow$laptop) both confirmed end-to-end.
